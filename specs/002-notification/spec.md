...
**Created**: 2026-04-13  
**Status**: Draft  
**Input**: User description: "xây dựng hệ thống thông báo gửi qua Email, SMS và Zalo sử dụng **Strategy Pattern + Factory Pattern**; interface `NotificationSender` là chiến lược (Strategy), mỗi kênh có implementation độc lập không phụ thuộc nhau; `NotificationFactory` chọn đúng chiến lược theo `ChannelType` (Factory); lưu trữ tạm JSON theo từng kênh; stub sender; API gửi và truy vấn thông báo"

**Design Patterns**:
- **Strategy Pattern** — `NotificationSender` là interface chiến lược; `EmailSender`, `SmsSender`, `ZaloSender` là các concrete strategy, mỗi class tồn tại độc lập, không import hay phụ thuộc nhau.
- **Factory Pattern** — `NotificationFactory` đóng vai trò factory: nhận `ChannelType` và trả về đúng strategy cần dùng, ẩn việc chọn implementation khỏi tầng nghiệp vụ.
...
**Why this priority**: Đây là chức năng cốt lõi. Strategy Pattern đảm bảo mỗi kênh gửi độc lập;
Factory Pattern chọn đúng strategy tại runtime — hai pattern kết hợp là nền tảng toàn bộ hệ thống.
Không có phần này thì P2 và P3 không thể triển khai.
...
- **FR-001** *(Strategy)*: Hệ thống MUST định nghĩa interface `NotificationSender` với phương thức
  `void send(Notification notification)` làm **Strategy interface** cho mọi kênh gửi.
  Đây là hợp đồng duy nhất mà các kênh phải tuân theo.
- **FR-002** *(Factory)*: Hệ thống MUST có `NotificationFactory` đóng vai trò **Factory**:
  nhận `ChannelType` và trả về đúng concrete strategy tương ứng tại runtime.
  Factory MUST ném lỗi rõ ràng khi gặp kênh không hỗ trợ.
- **FR-003** *(Independence)*: Ba concrete strategy `EmailSender`, `SmsSender`, `ZaloSender`
  MUST được cài đặt hoàn toàn độc lập: mỗi class chỉ phụ thuộc vào `NotificationSender` interface,
  **không import hay tham chiếu lẫn nhau**. Giai đoạn đầu sử dụng stub ghi log đầu ra.
- **FR-004** *(Open/Closed via Strategy)*: Hệ thống MUST cho phép thêm kênh mới bằng cách
  (1) bổ sung giá trị enum mới vào `ChannelType`,
  (2) tạo class mới implement `NotificationSender`,
  (3) đăng ký trong `NotificationFactory` —
  **không sửa đổi bất kỳ concrete strategy hiện có**.
...
- **FR-013** *(Strategy isolation)*: Runtime của một concrete strategy MUST không bị ảnh hưởng khi
  một concrete strategy khác thay đổi hoặc gặp lỗi — mỗi kênh hoạt động hoàn toàn biệt lập.
...
- **NotificationSender** *(Strategy interface)*: Hợp đồng hành vi gửi thông báo; mọi concrete
  strategy (EmailSender, SmsSender, ZaloSender) đều implement interface này và không biết
  sự tồn tại của nhau.
- **EmailSender / SmsSender / ZaloSender** *(Concrete Strategy)*: Ba implementation độc lập hoàn toàn;
  mỗi class chỉ phụ thuộc vào `NotificationSender` interface, không có cross-dependency.
- **NotificationFactory** *(Factory)*: Chọn và trả về đúng concrete strategy theo `ChannelType`
  tại runtime; áp dụng nguyên tắc Open/Closed — thêm kênh mới không sửa factory logic hiện có.
...
- **SC-004**: Tỷ lệ thêm kênh mới thành công mà không sửa đổi bất kỳ concrete strategy hiện có
  đạt 100% (kiểm tra bằng cách bổ sung kênh thử nghiệm — chỉ thêm class mới + đăng ký factory).
- **SC-006**: 100% concrete strategy (EmailSender, SmsSender, ZaloSender) không có cross-dependency
  lẫn nhau — xác minh qua phân tích import trong code review.
